...
_qwen3_5_mappings = [
    # Full-attention layers (self_attn): input_layernorm → q/k/v projections
    AWQMapping(
        "re:.*layers\\.(3|7|11|15|19|23|27|31|35|39|43|47|51|55|59|63)\\.input_layernorm$",
        ["re:.*self_attn.q_proj$", "re:.*self_attn.k_proj$", "re:.*self_attn.v_proj$"],
    ),
    # Full-attention layers (self_attn): v_proj → o_proj
    AWQMapping("re:.*self_attn.v_proj$", ["re:.*self_attn.o_proj$"]),
    # Linear-attention layers: input_layernorm → in_proj_qkv, in_proj_z
    AWQMapping(
